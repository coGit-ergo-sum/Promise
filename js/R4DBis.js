...

{
	...
	for(let i = 0; i < n; i++){
		...
		R4DBis.pbs[i] = new ProgresBar2('ProgressBar' + i);

		R4DBis.pbs[i].probabilities.resolve = 50;
		R4DBis.pbs[i].probabilities.error   = 10;		
		R4DBis.pbs[i].probabilities.reject  = 20;
		R4DBis.pbs[i].probabilities.timeout = 20;
	
		R4DBis.pbs[i].appendTo('tdR4DBisProgressBar');		
		R4DBis.pguis[i] = new PromiseGUI(1);	
		debugger;
		let pguiElement = R4DBis.pguis[i].getTopElement();
		R4DBis.pbs[i].appendPGui(pguiElement);	
	}	
	
	R4DBis.pgui = new PromiseGUI(1);
	R4DBis.pgui.appendTo('trR4DBis');



	function btnR4DBisGO_onclick(btn){
		...
		for(let i = 0; i < n; i++){
			...
			var pb = R4DBis.pbs[i];
			let pgui = R4DBis.pguis[i];
			pb.reset();	
			pgui.reset();	
			
			function executorR4DBis(resolve, reject) {
			...
			let promise = new Promise(executorR4DBis);
			promise
		...
		promiseAll.then(R4DBis.pgui.onResolve,  R4DBis.pgui.onReject)
		.finally(R4DBis.pgui.onFinally)
		.catch(R4DBis.pgui.onCatch);
	}
...
// 		R4DBis.pbs[i].probabilities.resolve = 70;
// 		R4DBis.pbs[i].probabilities.error   =  0;		
// 		R4DBis.pbs[i].probabilities.reject  = 20;
// 		R4DBis.pbs[i].probabilities.timeout = 10;
	
// 		R4DBis.pbs[i].appendTo('tdR4DBisProgressBar');
// 		//tools.append2Demo(R4DBis.pbs[i], 'tdR4DBisC2');
// 		//pbR4DBis.appendTo('tdR4DBisC2')	
		
// 		R4DBis.pguis[i] = new PromiseGUI(1);	
// 		//R4DBis.pguis[i].prependTo(R4DBis.pguis[i]);
// 		debugger;
// 		let html1 = R4DBis.pguis[i].getHTML();
// 		let html2 = R4DBis.pbs[i].getHTML();
// 		R4DBis.pbs[i].prependTd(R4DBis.pguis[i].$td);
...
// 	R4DBis.pgui = new PromiseGUI(1);
// 	R4DBis.pgui.append2ndChild('trR4DBis');
